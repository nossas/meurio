class AddServerForTheNewMultitudeDatabase < ActiveRecord::Migration
  def up
    if Rails.env.production? || Rails.env.staging?
      ...
      execute "ALTER SERVER multitude OPTIONS (SET dbname '#{ENV["MULTITUDE_DBNAME"]}', SET host '#{ENV["MULTITUDE_DBHOST"]}');"
      execute "ALTER USER MAPPING FOR #{ENV["DB_USERNAME"]} SERVER multitude OPTIONS (SET user '#{ENV["MULTITUDE_DBUSER"]}', SET password '#{ENV["MULTITUDE_DBPASS"]}');"
    end
  end
