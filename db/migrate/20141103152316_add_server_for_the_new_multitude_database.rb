class AddServerForTheNewMultitudeDatabase < ActiveRecord::Migration
  def up
    if Rails.env.production? || Rails.env.staging?
      ...
      execute "ALTER SERVER multitude FOREIGN DATA WRAPPER postgres_fdw OPTIONS (dbname '#{ENV["MULTITUDE_DBNAME"]}', host '#{ENV["MULTITUDE_DBHOST"]}');"
      execute "ALTER USER MAPPING FOR #{ENV["DB_USERNAME"]} SERVER multitude OPTIONS (user '#{ENV["MULTITUDE_DBUSER"]}', password '#{ENV["MULTITUDE_DBPASS"]}');"
    end
  end
